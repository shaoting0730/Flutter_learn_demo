...
baseComponent({
  ...
  methods: {
    ...
    onTouchEnd(e) {
      ...
      const { distance, scrollTop } = this.properties;
      this.start = false
      this.isMoved = false

      if (this.diffY <= 0 || this.direction !== 'Down' || this.isRefreshing() || this.isLoading() || scrollTop - distance > 0) return
    ...
    onScroll(n) {
      ...
      // 获取节点高度
      const query = this.parent ? wx.createSelectorQuery().in(this.parent) : wx.createSelectorQuery()
      const component = query.select(`#${this.id}`)
      //console.log(component)
      //component.fields({ size: true });
      component.boundingClientRect((res) => {
        if (res) {
