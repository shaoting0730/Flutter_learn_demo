...
class TodayPlanPage extends StatefulWidget {
  ...
  TodayPlanPage({this.onRequestMessagePageRefresh, Key key}):super(key:key);
...
class TodayPlanPageState extends State<TodayPlanPage> {
  ...
  void initState() {
    ...
    DateTime dtStart = new DateTime(dtNow.year,dtNow.month,dtNow.day);
    DateTime dtEnd = dtStart.add(new Duration(days:1));

    _searchRequest = ShowingScheduleSearch(StartCreateTime: DateTimeToTicks(dtStart.toUtc()), EndCreateTime: DateTimeToTicks(dtEnd.toUtc()), SortBy: 'asc', PageIndex: 0, PageSize: 50, RequestStatus: 0);
    setState(() {
      _loading = true;
    });
  ...
  void _requestRefresh(bool loading, bool readMore) async {
    ...
    if(showingRequestScheduler != null) {
      showingRequestScheduler.ListObjects.forEach((element) {
        ...
        DateTime startTime = TicksToDateTime(element.ScheduledStartTime);
        DateTime endTime = TicksToDateTime(element.ScheduledEndTime);
    ...
    if(mounted) {
      setState(() {
        ...
        _enablePullUp = false;///showingRequestScheduler.TotalCount > this.showingRequests.length;
      });
    }
  }
