...
class MessageReceivedPage extends StatefulWidget {
  ...
  MessageReceivedPage({this.onRequestMessagePageRefresh, Key key, this.parentPage}):super(key:key);

  @override
  State<StatefulWidget> createState() {
    return MessageReceivedPageState();
  }

}

class MessageReceivedPageState extends State<MessageReceivedPage> {
            ...
            child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if(index < showingRequests.length) {
                  ...
                  return Container(
                    child: MessageCell(message: model, disable: false, MessageType: 0, onRequestMessagePageRefresh: widget.onRequestMessagePageRefresh,),
                  );
                }
              },
            )
  ...
  void _requestRefresh(bool loading, bool readMore) async {
    ...
    // 查询  plan
    PagedListShowingRequestModel showingRequestScheduler = await UserServerApi().SearchShowingRequestForApproval(context,_searchBoxKey.currentState==null?'':_searchBoxKey.currentState.text(), _curPageIndex, 20);
