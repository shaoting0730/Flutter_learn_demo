...
>  其中，在 Flutter 中，Widget 和 Element 并不是一对多关系，而是“一对一实例映射”。每个 Widget 实例在插入树时都会创建一个对应的 Element。不过由于 Widget 是不可变且频繁重建的，而 Element 会被复用，所以从生命周期角度看，是“多个 Widget 实例复用同一个 Element”,widget不可变（只能销毁并生产新的，因为较为轻量，所以开销不大），element和RenderObject可变 <br/>
